...
class CommandHandlerTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def testRandomChoiceHappy(self):
        self.assertEqual(
            await randomChoice.randomChoice(
        ...
        self.assertEqual(
            await randomChoice.randomChoice(
                *testHelper.parseRawCommand(
                    "Lord of the Rings, everything everywhere all at once, friday the 13th, lion king"
                )
            ),
    ...
    async def testRandomChoiceSad(self):
        self.assertEqual(
            await randomChoice.randomChoice(), "You have given me no choice"
        )
        self.assertEqual(
            await randomChoice.randomChoice(
                *testHelper.parseRawCommand("   ,  , , ,  ,,,  , , , , ")
    ...
    async def testGuildCreated(self):
        with patch("datetime.datetime") as pachedDatetime:
            ...
            pachedDatetime.now.return_value = datetime(
                2023, 5, 14, 13, 45, 20, 12, timezone.utc
            )
    ...
    async def testUserJoined(self):
        with patch("datetime.datetime") as pachedDatetime:
            ...
            pachedDatetime.now.return_value = datetime(
                2023, 3, 18, 23, 12, 20, 12, timezone.utc
            )
